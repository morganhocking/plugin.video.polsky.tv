...
IPTV_DOMAIN = 'iptv.polsky.tv' 
# http://iptv.polsky.tv/api/json/%s
...
try:											# Let's see if cookielib is available
	...
	xbmc.log('[polskyTV] cookielib is not available..')
	pass
else:
...
try:
	...
	xbmc.log('[polskyTV] module json is not available. using demjson')
	import demjson
...
class polsky:
	...
	def _request(self, cmd, params, inauth = None):
		...
		xbmc.log('[polsky.TV] REQUESTING: %s' % url)
		...
		xbmc.log('[polsky.TV] Got %s' % rez, level=xbmc.LOGDEBUG)
		
		try:
			res = JSONDECODE(rez)
		except:
			xbmc.log('[polsky.TV] Error.. :(')
		...
		if COOKIEJAR != None:
			xbmc.log('[polsky.TV] Saving cookies: %s' % COOKIEFILE)
		...
		if 'sid' in response:
			...
			if COOKIEJAR != None:
				try:
					...
					xbmc.log('[polsky.TV] pipec kakaja strannaja shnyaga slu4ilas, pacany!', xbmc.LOGWARNING)
	...
	def getLast(self):
		if self.last_list and 'ttl' in self.last_list:
			if int(self.last_list['ttl']) < time():
				xbmc.log('[polsky.TV] Last list expired')
				self.last_list = None
		if not self.last_list:
			...
			try:
				...
				xbmc.log('[polsky.TV] Error loading last list %s' % e)
				res = self.getChannelsList()
			else:
				xbmc.log('[polsky.TV] last list loaded')
			f.close()
	...
	def getChannelsList(self):
		if self.channels_ttl < time():
			...
			try:
				...
				xbmc.log('[polsky.TV] Error saving last list %s' % e)
			else:
				xbmc.log('[polsky.TV] last list stored')
			f.close()
	...
	def getVideoList(self, mode, page, pagesize=25, search={}):
		if pagesize == 'all':
			pagesize = 999
			page = 1
			...
			xbmc.log('[polsky.TV] pagesize set to %s to reflect "all" param' % pagesize)
		...
		for vod in result['rows']:
			if 'poster' in vod:
				...
				icon = ''
			xbmc.log('[polsky.TV] VOD item: %s' % vod, level=xbmc.LOGDEBUG)
			res.append({
				'title':		vod['name'],
	...
	def getSettingsList(self):
		...
		xbmc.log('[polsky.TV] settings: %s' % res)
		return res
