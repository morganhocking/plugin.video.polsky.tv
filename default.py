...
PLUGIN_ID = 'plugin.video.polsky.tv'
...
def Archive(plugin, id, params, cname):
	...
	goback_title = '[B]< [ %s] %s[/B]' % (day_ago.strftime('%a, %d %B'),cname)
	...
	uri = sys.argv[0] + '?mode=Archive&channel=%s&dt=%s&can_play=%s&cname=%s' % (id, day_ago.toordinal(), params['can_play'],cname)
	xbmc.log(uri) 
	...
	for prog in epg:
		...
		if can_play:
			if progStart > time.time():
				can_play = False
		title = '[B][COLOR white]%s[/COLOR][/B][COLOR tan] %s[/COLOR] %s' % (timeLabel, prog['title'], prog['info'])
		if progStart < time.time():
			if currentProg == False:
				if len(epg) > counter:
					p = epg[counter]
					if int(p['time']) > time.time():
                                                title = '[B] [COLOR white]%s[/COLOR][COLOR blue] %s [/COLOR] %s[/B]' % (timeLabel, prog['title'], prog['info'])
						currentProg = counter
			pass
		else:
                        title = '[I][COLOR gray]%s %s %s[/COLOR][/I]' % (timeLabel, prog['title'], prog['info'])
	...
	goahead_title = '[B]> [ %s ] %s[/B]' % (day_forward.strftime('%a, %d %B'),cname)
	...
	uri = sys.argv[0] + '?mode=Archive&channel=%s&dt=%s&can_play=%s&cname=%s' % (id, day_forward.toordinal(), params['can_play'],cname)
	...
	for channel in channels:
		...
		if channel['is_video']:
			...
			if str(channel['id']) in favs:
				overlay = 7
			theicon = os.path.join(addon.getAddonInfo('path'), 'resources', 'icons') + channel['icon'][29:33] +'.png'
			item=xbmcgui.ListItem(channel['subtitle'], channel['title'], iconImage=channel['icon'], thumbnailImage=theicon)
			color = channel['color']
			xbmc.log(theicon) 
			xbmc.log("this is a channels")
			...
			if channel['have_epg'] != False:
				...
				uri2 = sys.argv[0] + '?mode=Archive&channel=%s&can_play=%s&cname=%s' % (channel['id'], channel['have_archive'],channel['title'])
...
def VideoInfo(plugin, params):
	...
	for video in film['videos']:
		...
		icon = 'http://iptv.polsky.tv%s' % film['poster']
...
PLUGIN_CORE = iptv.polsky(USERNAME, USERPASS, PLUGIN_ID, SID, SID_NAME)
...
if PLUGIN_CORE.testAuth() == False:
	...
	if mode == 'WatchTV':
		...
		Archive(PLUGIN_CORE, channel, params, cname)
